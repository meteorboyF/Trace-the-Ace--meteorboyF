...
| 6 | **Cloud compute / cloud storage is acceptable** — confirmed again in a dedicated thread (2026-07-21) approving a private, access-controlled rented GPU VM. Conditions: data stays under our control, is not made public, and **is not used by the provider for model training or any other secondary purpose**. | ✅ Colab Pro+ usage is fine, subject to those conditions. Verify Google's terms for the account tier in use. |
| 6b | **"Any models used still need to meet the open licensing requirement. This applies to models used for development."** (stated twice: 2026-06-25 and 2026-07-21) | ⚠️ **OPEN:** does this cover AI *coding assistants*, or only models used as pipeline ingredients? See [`OPEN_QUESTION_ai_assistant.md`](OPEN_QUESTION_ai_assistant.md) — recommend disclosing and asking. |
